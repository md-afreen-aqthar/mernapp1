...
const [ecommerceCat,setecommerceCat]=useState([]);

const [ecommerceItem,setecommerceItem]=useState([]);

const loadData= async()=>{
  let response=await fetch("http://localhost:5000/api/ecommerceData",{
    method:"POST",
    headers:{
...
setecommerceItem(response[0])
setecommerceCat(response[1])
...
<div className="d-flex justify-content-center">
  ...
  <button className="carousel-control-next" type="button" data-bs-target="#carouselExampleControls" data-bs-slide="next">
    ...
    <div className="container">
      {
        ecommerceCat.length > 0
        ? ecommerceCat.map((data) => {
          return (
          <div className="row mb-3">
            ...
            {ecommerceCat.length > 0? ecommerceItem.filter ((item)=> (item.CategoryName === data.CategoryName)  && (item.name.toLowerCase().includes(search.toLocaleLowerCase())))
            .map(filterItems=>{
              return (
                <div key={filterItems._id} className="col-12 col-md-6 col-lg-3">

                  <Card ecommerceName={filterItems.name}
